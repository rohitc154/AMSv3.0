...
async function register(req, res, next) {
  try {
    ...
    const existingPending = await PendingRegistration.findOne({ email }).select('lastSentAt').lean();
    if (existingPending?.lastSentAt) {
      ...
      if (since < otpResendCooldownMs()) {
        return res.status(429).json({ message: 'OTP recently sent. Please wait and try again.' });
      }
    }
...
async function verifyRegistrationOtp(req, res, next) {
  try {
    const { error, value } = verifyRegistrationOtpSchema.validate(req.body, { abortEarly: false });
    if (error) {
      return res.status(400).json({ message: error.details.map((d) => d.message).join(', ') });
    }
    ...
    const pending = await PendingRegistration.findById(value.pendingId).select('+otpHash +passwordHash');
    if (!pending) {
    ...
    if (pending.attempts >= 5) {
      return res.status(429).json({ message: 'Too many OTP attempts. Register again.' });
    }
...
async function resendRegistrationOtp(req, res, next) {
  try {
    const { error, value } = resendRegistrationOtpSchema.validate(req.body, { abortEarly: false });
    if (error) {
      return res.status(400).json({ message: error.details.map((d) => d.message).join(', ') });
    }
    if (!mongoose.isValidObjectId(value.pendingId)) {
    ...
    const pending = await PendingRegistration.findById(value.pendingId).select('+otpHash');
    if (!pending) {
      return res.status(404).json({ message: 'Pending registration not found or expired' });
    }
    const since = pending.lastSentAt ? Date.now() - pending.lastSentAt.getTime() : Infinity;
    if (since < otpResendCooldownMs()) {
      return res.status(429).json({ message: 'OTP recently sent. Please wait and try again.' });
    }
...
async function forgotPassword(req, res, next) {
  try {
    ...
    const existing = await PasswordResetOtp.findOne({ userId: user._id }).select('lastSentAt').lean();
...
async function resetPassword(req, res, next) {
  try {
    ...
    const resetRow = await PasswordResetOtp.findOne({ userId: user._id }).select('+otpHash');
    if (!resetRow) {
      return res.status(400).json({ message: 'Invalid email or OTP' });
    }
